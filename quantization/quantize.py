...
def _observer_forward_pre_hook(self, input):
    r"""Forward hook that calls observer on the output
    """
    self.activation_post_process(*input)
...
def add_observer_lala(module):
    for child in module.children():
        if type(child) == nnq.FloatFunctional:
            if hasattr(child, 'qconfig') and child.qconfig is not None:
                child.activation_post_process = child.qconfig.activation()
        else:
            add_observer_lala(child)
...
def prepare_lala(model, qconfig_dict=None, inplace=False):
